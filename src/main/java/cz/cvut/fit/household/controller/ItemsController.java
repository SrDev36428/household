...
public class ItemsController {
    ...
    public String addItem(@PathVariable Long householdId,
        ...
        log.debug("Creating a new item - {} in location with id - {} of household with id - {}", item, locationId,
                householdId);
        ...
        try {
            if (!item.getMaxQuantity().isEmpty() && !item.getCurrentQuantity().isEmpty()) {
                ...
                if (maxQuantity <= 0) {
                    result.rejectValue(MAX_QUANTITY_ATTR, ERROR_ATTR,
                            "Maximum quantity should not be higher than zero (0)");
                }
            ...
            log.info("Items - {} of {} multiples  have been created in location with id -{} of household with id - {}",
                    item, multipleCreation, locationId, householdId);
            model.addAttribute(HOUSEHOLD_ID_ATTR, householdId);
            ...
            eventPublisher.publishEvent(new OnInventoryChangeEvent(householdId,
                    "item:" + multipleCreation + ",title: " + item.getTitle() + ", status: created, "
                            + MAX_QUANTITY_ATTR + ": " + item.getMaxQuantity() + ", " + CURRENT_QUANTITY_ATTR + ": "
                            + item.getCurrentQuantity() + ", " + EXPIRATION_ATTR + ": " + item.getExpiration()
                            + ", location: " + location.getLocationPath()));
            return "redirect:/household/{householdId}/locations/{locationId}/";
        } catch (NumberFormatException e) {
            log.error("Error occurred during Creating item - {} location with id - {} of household - {}  :", item,
                    locationId, householdId, e);
    ...
    public String renderItemsList(@RequestParam(value = "sort", defaultValue = "title") String sort,
        ...
        items.sort((item1, item2) -> {
            switch (sort) {
                ...
                case "expiration":
                    return Comparator.nullsLast((Timestamp d1, Timestamp d2) -> d1.compareTo(d2))
                            .compare((Timestamp) item1.getExpiration(), (Timestamp) item2.getExpiration());
                case "-expiration":
                    return Comparator.nullsFirst((Timestamp d1, Timestamp d2) -> d2.compareTo(d1))
                            .compare((Timestamp) item1.getExpiration(), (Timestamp) item2.getExpiration());
                case "maxQuantity":
    ...
    public ModelAndView renderLocationItemsList(@PathVariable Long householdId,
        ...
        return new ModelAndView(REDIRECTION_HOUSEHOLD + householdId + "/" + LOCATION_ATTR + "s/" + locationId,
                (Map<String, ?>) model);
    }
    ...
    public ModelAndView deleteItem(@PathVariable Long householdId,
            ...
            Model model) {
        log.debug("Deleting item - {} in location with id - {} of household with id - {}", itemId, locationId,
                householdId);
        Household household = houseHoldService.findHouseHoldById(householdId)
                .orElseThrow(() -> {
                    log.error("Household with id -{} does not exist", householdId);
        ...
        log.info("Item with id - {} has been deleted in location with id -{} of household with id - {}", itemId,
                locationId, householdId);

        eventPublisher
                .publishEvent(
                        new OnInventoryChangeEvent(householdId, "title:" + item.getTitle()
                                + ", status: deleted, location: " + item.getLocation().getTitle()));
        model.addAttribute(HOUSEHOLD_ATTR, household);
    ...
    public ModelAndView deleteItemFromCategory(@PathVariable Long householdId,
            ...
            @PathVariable Long itemId,
            Model model) {
        log.debug("Deleting item - {} of category with id - {} of household with id - {}", itemId, categoryId,
                householdId);
        Household household = houseHoldService.findHouseHoldById(householdId)
                .orElseThrow(() -> {
                    log.error("Household with id - {} does not exist", householdId);
        ...
        itemService.deleteItemById(itemId);
        log.info("Item with id - {} of category with id - {} has been deleted in household with id - {}", itemId,
                categoryId, householdId);
        eventPublisher
                .publishEvent(
                        new OnInventoryChangeEvent(householdId,
                                "title:" + item.getTitle() + ", status: deleted, location: " + item.getLocation().getTitle()));
        model.addAttribute(HOUSEHOLD_ATTR, household);
        model.addAttribute(CATEGORY_ATTR, category);
        return new ModelAndView(REDIRECTION_HOUSEHOLD + householdId + "/" + CATEGORY_ATTR + "/" + categoryId,
                (Map<String, ?>) model);
    }
    ...
    public String performEditing(@PathVariable Long householdId,
            ...
            Model model) {
        log.debug("Editing an item with id - {} in location with id -{} of house hold -{} with new item - {}", itemId,
                locationId, householdId, updatedItem);
        Location location = locationService.findLocationById(locationId)
                .orElseThrow(() -> {
                    log.error("Location with id - {} does not exist", locationId);
                    return new NonExistentEntityException(LOCATION_WITH_ID + locationId + DOES_NOT_EXIST);
        ...
        String formattedOldDate = sdf.format(expiration); // e.g. 2025-04-24
        String formattedNewDate = sdf.format(updatedItem.getExpiration());

        if (!oldTitle.equals(updatedItem.getTitle())) {
            changedFields.put("title", oldTitle + " → " + updatedItem.getTitle());
        }
        if (oldCategory.trim() != updatedItem.getCategory().getTitle().trim()) {
            changedFields.put("category", oldCategory + " → " + updatedItem.getCategory().getTitle());
        }

        if (oldCurrentQuantity.compareTo(Double.parseDouble(updatedItem.getCurrentQuantity())) != 0) {
            changedFields.put("currentQuantity",
                    oldCurrentQuantity + " → " + updatedItem.getCurrentQuantity());
        }
        if (oldMaxQuantity.compareTo(Double.parseDouble(updatedItem.getMaxQuantity())) != 0) {
            changedFields.put("maxQuantity", oldMaxQuantity + " → " + updatedItem.getMaxQuantity());
        }

        if (!formattedOldDate.equals(formattedNewDate)) {
            changedFields.put("expiration", formattedOldDate + " → " + formattedNewDate);
        }

        // If no changes, redirect to the location page
        if (changedFields.isEmpty()) {
            log.info("No changes detected for item with id - {}", itemId);
            return "redirect:/household/{householdId}/locations/{locationId}";
        }
        ...
        log.info("Item with id - {} has been updated as {} in location with id - {} of household with id - {}", itemId,
                item, locationId, householdId);

        eventPublisher.publishEvent(
                new OnInventoryChangeEvent(householdId,
                        "status: updated, location: " + oldItem.getLocation().getTitle() + ", change:" + changedFields.toString()));
    ...
    public String performEditingFromCategory(@PathVariable Long householdId,
            ...
            Model model) {
        log.debug("Editing an item with id - {} of category with id -{} of household - {} with new item - {}", itemId,
                categoryId, householdId, updatedItem);
        Household houseHold = houseHoldService.findHouseHoldById(householdId)
                .orElseThrow(() -> {
                    log.error("Household with id - {} does not exist", householdId);
                    return new NonExistentEntityException(HOUSEHOLD_WITH_ID + householdId + DOES_NOT_EXIST);
        ...
        if (title.equals(updatedItem.getTitle())) {
            changedFields.put("title", title + " → " + updatedItem.getTitle());
        }
        if (prevCategory.trim() != updatedItem.getCategory().getTitle().trim()) {
            changedFields.put("category", prevCategory + " → " + updatedItem.getCategory().getTitle());
        }
        if (currentQuantity.compareTo(Double.parseDouble(updatedItem.getCurrentQuantity())) != 0) {
            changedFields.put("currentQuantity",
                    currentQuantity + " → " + updatedItem.getCurrentQuantity());
        }
        if (maxQuantity.compareTo(Double.parseDouble(updatedItem.getMaxQuantity())) != 0) {
            changedFields.put("maxQuantity", maxQuantity + " → " + updatedItem.getMaxQuantity());
        }
        if (!formattedOldDate.equals(formattedNewDate)) {
            changedFields.put("expiration", formattedOldDate + " → " + formattedNewDate);
        }
        ...
        Item item = itemService.updateItem(updatedItem, itemId);
        log.info("Item with id - {} of category with id - {} has been updated as {} in household with id - {}", itemId,
                categoryId, item, householdId);
        ...
        eventPublisher.publishEvent(
                new OnInventoryChangeEvent(householdId, "status: updated, location: " + oldItem.getLocation().getTitle()
                        + ",  change:" + changedFields.toString()));
        return "redirect:/household/{householdId}/category/{categoryId}";
    }
    ...
    public ModelAndView increaseQuantity(@PathVariable Long householdId,
        ...
        setEditingPage(householdId, locationId, itemId, model);
        log.debug(
                "Increasing quantity of an item with id -{}  in location with id - {} of household with id -{} with new item quantity - {}",
                itemId, locationId, householdId, itemDto);
        ...
        try {
            ...
            model.addAttribute(ITEM_ATTR, item);
            log.info(
                    "Increased quantity of an item with id -{}  in location with id - {} of household with id -{} with new item quantity - {}",
                    itemId, locationId, householdId, itemDto);
            eventPublisher.publishEvent(new OnInventoryChangeEvent(householdId,
                    "title:" + foundItem.getTitle() + ", status: increased, amount:"
                            + Double.valueOf(foundItem.getCurrentQuantity() - changedAmount) + ", nowAmount:"
                            + item.getCurrentQuantity() + ", location: " + location.getLocationPath()));
            return new ModelAndView(
                    REDIRECTION_HOUSEHOLD + householdId + "/" + LOCATION_ATTR + "s/" + locationId + "/items",
                    (Map<String, ?>) model);
        } catch (RuntimeException e) {
            log.error(
                    "Error occurred during increase quantity of of an item with id -{}  in location with id - {} of household with id -{} with new item quantity - {} : {} ",
                    itemId, locationId, householdId, itemDto, QUANTITY_ERROR_MSG);
    ...
    public ModelAndView decreaseQuantity(@PathVariable Long householdId,
        ...
        log.debug(
                "Decreasing quantity of an item with id -{}  in location with id - {} of household with id -{} with new item quantity - {}",
                itemId, locationId, householdId, itemDto);
        if (result.hasErrors()) {
            log.error(result.getAllErrors().toString());
            return new ModelAndView(RETURN_ITEM_QUANTITY_EDIT, (Map<String, ?>) model);
        ...
        try {
            ...
            log.info(
                    "Decreased quantity of an item with id -{}  in location with id - {} of household with id -{} with new item quantity - {}",
                    itemId, locationId, householdId, itemDto);

            eventPublisher.publishEvent(new OnInventoryChangeEvent(householdId,
                    "title:" + foundItem.getTitle() + ", status: decreased, amount:"
                            + Double.valueOf(foundItem.getCurrentQuantity() + changedAmount) + ", nowAmount:"
                            + item.getCurrentQuantity() + ", location: " + location.getLocationPath()));
            return new ModelAndView(
                    REDIRECTION_HOUSEHOLD + householdId + "/" + LOCATION_ATTR + "s/" + locationId + "/items",
            ...
            log.error(
                    "Error occurred during decrease quantity of of an item with id -{}  in location with id - {} of household with id -{} with new item quantity - {} : {} ",
                    itemId, locationId, householdId, itemDto, QUANTITY_ERROR_MSG);
            result.rejectValue(CURRENT_QUANTITY_ATTR, ERROR_ATTR, QUANTITY_ERROR_MSG);
            return new ModelAndView(RETURN_ITEM_QUANTITY_EDIT, (Map<String, ?>) model);
        }
    ...
    public ModelAndView increaseQuantityFromCategory(@PathVariable Long householdId,
        ...
        log.debug(
                "Increasing quantity of an item with id -{}  of category with id - {} of household with id -{} with new item quantity - {}",
                itemId, categoryId, householdId, itemDto);
        setEditingPageFromCategory(householdId, categoryId, itemId, model);
        ...
        try {
            ...
            Item item = itemService.increaseItemQuantity(itemDto.getCurrentQuantity(), itemId);
            log.info(
                    "Increased quantity of an item with id -{}  of category with id - {} of household with id -{} with new item quantity - {}",
                    itemId, categoryId, householdId, itemDto);
            model.addAttribute(HOUSEHOLD_ATTR, houseHold);
            model.addAttribute(HOUSEHOLD_ID_ATTR, householdId);
            model.addAttribute(CATEGORY_ATTR, category);
            ...
            eventPublisher.publishEvent(new OnInventoryChangeEvent(householdId,
                    "title:" + foundItem.getTitle() + ", status: increased, amount:"
                            + Double.valueOf(foundItem.getCurrentQuantity() - changedAmount) + ", nowAmount:"
                            + item.getCurrentQuantity() + ", location: " + foundItem.getLocation().getTitle()));
            return new ModelAndView(REDIRECTION_HOUSEHOLD + householdId + "/" + CATEGORY_ATTR + "/" + categoryId,
                    (Map<String, ?>) model);
        } catch (RuntimeException e) {
            log.error(
                    "Error occurred during decrease quantity of of an item with id -{}  of category with id - {} of household with id -{} with new item quantity - {} : {} ",
                    itemId, categoryId, householdId, itemDto, QUANTITY_ERROR_MSG);
            result.rejectValue(CURRENT_QUANTITY_ATTR, ERROR_ATTR, QUANTITY_ERROR_MSG);
            return new ModelAndView(RETURN_QUANTITY_EDIT_CATEGORY, (Map<String, ?>) model);
        }
    }
    ...
    public ModelAndView decreaseQuantityFromCategory(@PathVariable Long householdId,
        ...
        try {
            ...
            Item item = itemService.decreaseItemQuantity(itemDto.getCurrentQuantity(), itemId);
            log.info(
                    "Decreased quantity of an item with id -{}  of category with id - {} of household with id -{} with new item quantity - {}",
                    itemId, categoryId, householdId, itemDto);
            model.addAttribute(HOUSEHOLD_ATTR, houseHold);
            model.addAttribute(HOUSEHOLD_ID_ATTR, householdId);
            model.addAttribute(CATEGORY_ATTR, category);
    ...
    public String relocateItem(@PathVariable Long householdId,
            ...
            Model model) {
        log.debug("Relocating item with id - {}  in location  with id - {} of household with id - {}", itemId,
                locationId, householdId);
        Household household = houseHoldService.findHouseHoldById(householdId)
                .orElseThrow(() -> {
                    log.error("Household with id - {} does not exist", householdId);
        ...
        log.info("Item with id - {} has been relocated to  location with id -{} of household with id - {}", itemId,
                locationId, householdId);
    ...
    public String relocateItemFromCategory(@PathVariable Long householdId,
            ...
            @ModelAttribute ItemRelocationDTO itemDto,
            Model model) {
        log.debug("Relocating item with id - {}  of category  with id - {} of household with id - {}", itemId,
                categoryId, householdId);

        Item item = itemService.relocateItem(itemDto);
        log.info("Item with id - {} has been Relocated to  category with id -{} of household with id - {}", itemId,
                categoryId, householdId);
        Location oldLocation = locationService.findLocationById(item.getLocation().getId())
                .orElseThrow(() -> {
                    ...
                    return new NonExistentEntityException(
                            LOCATION_WITH_ID + item.getLocation().getId() + DOES_NOT_EXIST);
                });
        Household houseHold = houseHoldService.findHouseHoldById(householdId)
                .orElseThrow(() -> {
                    log.error("Household with id - {} does not exist", householdId);
